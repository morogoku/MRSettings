...
import com.qfcolorpicker.renderer.ColorWheelRenderOption;
...
public class ColorPickerView extends View {
	...
	private void initWith(Context context, AttributeSet attrs) {
		...
		pickerTextColor = typedArray.getInt(R.styleable.ColorPickerPreference_pickerColorEditTextColor, 0xff222222);
	...
	public void setColor(int color, boolean updateText) {
		int tmp = getSelectedColor(); // grs fix
		setInitialColor(color, updateText);
		updateColorWheel();
		if(getSelectedColor()!=tmp) callOnColorChangedListeners(tmp,getSelectedColor()); // grs fix
		invalidate();
	}
