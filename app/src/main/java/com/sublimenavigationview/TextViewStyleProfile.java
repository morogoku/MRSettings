...
public class TextViewStyleProfile {
    ...
    //grxgrx
    private ColorStateList mHintTextColor, mHeaderTextcolor, mHeaderHintTextColor;
    ...
    private void setDefaultItemTextColor() {
        ...
        if (mContext.getTheme().resolveAttribute(android.R.attr.textColorPrimary, value, true)) {
            ...
            if (mContext.getTheme().resolveAttribute(R.attr.colorPrimary, value, true)) {
                ...
                //grx
                int menuTextColor=defaultColor;
                if (mContext.getTheme().resolveAttribute(R.attr.svn_menuTextColor, value, true)) {
                    menuTextColor=value.data;
                }
                mTextColor = new ColorStateList(new int[][]{DISABLED_STATE_SET, CHECKED_STATE_SET, EMPTY_STATE_SET},
                        new int[]{baseColor.getColorForState(DISABLED_STATE_SET, defaultColor), colorPrimary, menuTextColor});
            }
        }
    ...
    /*    if (mTextColor == null) {
        ...
        if(mTextColor==null) mTextColor=getDefaultcolorStateList();
        ...
        if (mHintTextColor == null) {
            ...
            if (mContext.getTheme().resolveAttribute(android.R.attr.textColorPrimary, value, true)) {
                ...
                if (mContext.getTheme().resolveAttribute(R.attr.colorPrimary, value, true)) {
                    int colorPrimary = value.data;
                    int defaultColor = baseColor.getDefaultColor();
                    int hintcolor = defaultColor;
                    if (mContext.getTheme().resolveAttribute(R.attr.svn_hintTextColor, value, true)) {
                        hintcolor=value.data;
                    }

                    mHintTextColor = new ColorStateList(new int[][]{DISABLED_STATE_SET, CHECKED_STATE_SET, EMPTY_STATE_SET},
                            new int[]{baseColor.getColorForState(DISABLED_STATE_SET, defaultColor), colorPrimary, /*defaultColor*/ hintcolor});
                }
            }
        }

        if (mHintTextColor == null) mHintTextColor = getDefaultcolorStateList();
