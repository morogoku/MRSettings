...
//            Log.e(TAG, "'setItemStyleProfile(TextViewStyleProfile)' was " + "called with a 'null' value");
    ...
    //grxgrx
    public ColorStateList getCenteredTextTintList(){

        TypedValue value = new TypedValue();
        if (mContext.getTheme().resolveAttribute(android.R.attr.textColorPrimary, value, true)) {
            ColorStateList baseColor = mContext.getResources().getColorStateList(value.resourceId);
            if (mContext.getTheme().resolveAttribute(R.attr.menucenteredtextcolor, value, true)) {
                int colorPrimary = value.data;
                int defaultColor = baseColor.getDefaultColor();
                mCenteredTintiList = new ColorStateList(new int[][]{DISABLED_STATE_SET, CHECKED_STATE_SET, EMPTY_STATE_SET},
                        new int[]{baseColor.getColorForState(DISABLED_STATE_SET, defaultColor), colorPrimary, defaultColor});
            }
        }

        if(mCenteredTintiList==null) setDefaultCheckableItemTintList();
        return mCenteredTintiList;
    }
    ...
    private void setDefaultCheckableItemTintList() {
        ...
        if (mContext.getTheme().resolveAttribute(android.R.attr.textColorPrimary, value, true)) {
            ColorStateList baseColor = mContext.getResources().getColorStateList(value.resourceId);
            if (mContext.getTheme().resolveAttribute(/*R.attr.colorPrimary*/R.attr.colorAccent, value, true)) {
                int colorPrimary = value.data;
                int defaultColor = baseColor.getDefaultColor();
